...
class BCJR(object):
    ...
    def createGammaMat(self,La,Es_N0=1):
        ...
        for l in xrange(self.h):
            ...
            for state in xrange(rows):
                ...
                self.gamma[state,l,0] = Lc_d2*np.dot(r_seq,outBits)
        ...
        # ---- gamma for terminating bits ---- #
        for l in xrange(self.h,self.K):
            # Extract the current r sequence
            r_seq = self.r[l*self.n:l*self.n+(self.n)]
            ...
            for state in xrange(rows):
                ...
                self.gamma[state,l,1] = Lc_d2*np.dot(r_seq,outBits)
    ...
    def createBetaMat(self):
        ...
        # make the matrix
        self.beta = np.full((2**self.v,self.K),-50,dtype=np.float64)
